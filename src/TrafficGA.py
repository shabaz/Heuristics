...
class Traffic(object):
    ...
    def setAirplanes(self):
        ...
        traffic = [0 for x in range(self.initialPopSize)]
        ...
        while population>1:
            ...
            for i in range(0,population):
                tour = tours[i]
                traffic[i] = Airplane(0,self.cities[0],self.cities[0],self.COLOR[0])
            ...
            while choosingParents:
                x = random.randrange(0,population)
                y = random.randrange(0,population)
                if traffic_points[x]>traffic_points[y]:
                    parents.append(tours[x])
                else:
                    parents.append(tours[y])
                parentsLeft -= 1

                if parentsLeft == 0:
                    choosingParents = False

            makeChildren = True
            childrenLeft = numberOfChildren
            children = []
