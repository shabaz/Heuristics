...
from TrafficFrameVis import TrafficFrame
...
class Traffic(object):
    ...
    def __init__(self, homebase):
        ...
        self.temperature = 100000 * self.NUMBER_OF_AIRPLANES
        self.last_accepted_tours = None
        self.last_accepted_score = 0
        self.setAirplanes()

        self.frame = TrafficFrame(self)
    ...
    def setAirplanes(self):
        ...
        for i in range(len(traffic)):
            ...
            traffic[i] = Airplane(i, self.cities[self.homebase], self.cities[shifted_tour[0]], self.COLOR[i])
            ...
            for j in shifted_tour[1:]+[shifted_tour[0]]:
                ...
                flight = Flight(passengers,
                        self.currentTime, 
                        prev_city, 
                        (float(prev_city.getDistanceTo(next_city.getIndex())) / float(traffic[i].getAirplaneSpeed()) * 60.0) + self.currentTime, 
                        next_city, 
        ...
        if choose_transition(traffic_points, self.last_accepted_score, self.temperature):
            self.last_accepted_score = traffic_points
            self.traffic = traffic
            self.last_accepted_tours = tours
