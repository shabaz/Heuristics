...
int calculate_cost(std::vector<int>(&routes)[6])
{
...
std::vector<int> best_routes[6];
int best_updates = 0;

void check_tree(std::vector<int>(& routes)[6], float time_so_far[6], float cost_so_far[6], int depth[6], int current_route)
{
    int prev_route = (current_route + 5)%6;
    if (time_so_far[prev_route] > 1200) {
        return;
    }
    ...
    for (int i = 0; i < 6; i++)
    {
        float time_left = 1200 - time_so_far[i] + 60;
        float km_in_air = (time_left/1.25) / 60.0 * 800.0;
        float max_potential_extra_cost = km_in_air * 199;
        max_cost += cost_so_far[i] + max_potential_extra_cost;
    ...
    for (int i = 0; i < 6; i++) {
        if (routes[i][0] == routes[i].back() || !check_valid_route(routes[i])) {
            valid_routes = false;
            break;
        }
    }
    ...
    if (valid_routes) {
        int cost = calculate_cost(routes);
        if (cost > best_cost) {
            best_cost = cost;
            for (int i = 0; i < 6; i++) {
                best_routes[i] = routes[i];
            }
            std::cout << "new best cost " << best_cost << " with route [";
            for (auto best_route: best_routes) {
                std::cout << "[";
    ...
    for (int i = 0; i < 28; i++)
    {
        int distance = DISTANCES[routes[current_route].back()][i];
        if (distance > 3199 || i == routes[current_route].back())
            continue;
        float additional_time_in_air = distance / 800.0 * 60;
        float additional_naive_tanking_time = distance / 3199.0 * 60;
        int passengers = global_passengers[routes[current_route].back()][i];
        if (passengers > 199)
            passengers = 199;
        global_passengers[routes[current_route].back()][i] -= passengers;
        routes[current_route].push_back(i);
        float next_time_so_far[6];
        float next_cost_so_far[6];
        int next_depth[6];
        for (int i = 0; i < 6; i++) {
            next_time_so_far[i] = time_so_far[i];
            next_cost_so_far[i] = cost_so_far[i];
            next_depth[i] = depth[i];
        }
        next_time_so_far[current_route] += additional_naive_tanking_time + additional_time_in_air + 60;
        next_cost_so_far[current_route] += passengers * distance;
        next_depth[current_route]++;
        check_tree(routes, next_time_so_far, next_cost_so_far, next_depth, (current_route+1)%6);
        routes[current_route].pop_back();
        global_passengers[routes[current_route].back()][i] += passengers;
    }
    if (depth[current_route] < 1)
        std::cout << "current route " << current_route << " depth " << depth[current_route] << " checked " << checked << " valid " << valid << std::endl;
}

int main(int argc, char* argv[])
{
    int homebase = 0;
    std::vector<int> routes[6];
    for (auto& route: routes) {
        route.reserve(20);
    ...
    check_tree(routes, time_so_far, cost_so_far, depth, 0);
